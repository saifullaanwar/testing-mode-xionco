...
socket.on('newadditemadminlist',async function(datas) {
  ...
  // parsing dataset JSON, aman jika null atau invalid
  let dataarray=window.dataall;

  try {
    //dataarray = itemalls.dataset.json ? JSON.parse(itemalls.dataset.json) : { dataadminlist: [] };
    // pastikan ada properti dataadminlist
    if (!Array.isArray(dataarray.dataadminlist)) {
      dataarray.dataadminlist = [];
    }
  } catch (e) {
    console.error('JSON invalid, membuat array baru:', e);
    dataarray = { dataadminlist: [] };
  }

  
  var listmessage_notif=[];
  // tambahkan data baru di awal array
  for (let i = 0; i < datas.length; i++) {
    const element = datas[i];
    dataarray.dataadminlist.unshift(element);
    var message_create=`Transakasi baru ID : ${element.id_transaksi}, ${element.item} ${element.colorvariant==''?'':`, ${element.colorvariant}`}`;
    listmessage_notif.push(message_create);

    updatedataNotif(message_create);
  }

  // update dataset
  // itemalls.dataset.json = JSON.stringify(dataarray);
      ...
      if (element.id_transaksi==datas.id_transaksi) {
        if (element.status.toLowerCase()!='selesai'&&element.status.toLowerCase()!='dibatalkan') {
          if (datas.approveorreject=='approve') {
            ...
            window.dataall=dataarray;  ////document.getElementById('itemalls').textContent=JSON.stringify(dataarray);
