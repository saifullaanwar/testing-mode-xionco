...
function showtabel(array, awalnotif = false) {
    ...
    // ==============================
    // 🔹 PROSES DUPLIKASI HISTORY "Standby Pengiriman" MENJADI "New Standby Pengiriman"
    // ==============================
    
    const hasil3 = hasil2.map(item => {
        if (item.status === "Standby Pengiriman") {// && Array.isArray(item.history)
            ...
            const index = newHistory.findIndex(h => h.details?.status === "Standby Pengiriman");
            console.log('ini jalan ga?');
            
            if (index !== -1) {
                const original = newHistory[index];
                const copy = JSON.parse(JSON.stringify(original));

                // ubah nilai-nilai pada duplikat
                copy.details.status = "New Standby Pengiriman";
                copy.details.ketstatus = `Auto-created New Standby Pengiriman dari Standby Pengiriman`;
                copy.details.tglinputmili = Number(item.delivereddate_mili);

                // ubah tampilan tanggal agar jelas
                copy.tgl = item.delivered_date + " (Auto New SP)";

                // sisipkan persis setelah posisi aslinya
                newHistory.splice(index + 1, 0, copy);
            }
    ...
    console.log("hasil3 (dengan New Standby Pengiriman)");
    console.log(hasil3);
    ...
    hasil3.forEach((item) => {
        ...
        let tglmili_statusnow = null;

        // 🔸 Jika status sekarang adalah Standby Pengiriman → ambil dari New Standby Pengiriman
        if (item.status === "Standby Pengiriman") {
            const newSP = historyArray.find(h => h.details?.status === "New Standby Pengiriman");
            if (newSP) tglmili_statusnow = newSP.details.tglinputmili;
        }

        // 🔸 Fallback jika tidak ada New SP → ambil history terakhir valid (bukan edit)
        if (!tglmili_statusnow) {
            const lastValidHistory = historyArray.find(
                (entry) => entry.tindakan.toLowerCase() !== "edit item"
            );
            tglmili_statusnow = lastValidHistory?.details?.tglinputmili || null;
        }
