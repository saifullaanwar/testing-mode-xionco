...
function cekNcreatTab(array, totalorongoing) {
    ...
    //             // ✅ Khusus role Driver di status Standby Pengiriman
    //             else if (statusName === "Standby Pengiriman" && roleName === "Driver") {
    //                 start = "New Standby Pengiriman"; // 🔥 ambil dari duplikasi history
    ...
    for (const [roleName, roleData] of Object.entries(ongoingLate)) {
        for (const [statusKey, statusData] of Object.entries(roleData)) {
            ...
            for (const item of statusData.detail) {
                ...
                if (roleName === "Driver" && statusName === "Standby Pengiriman") {
                    start = "New Standby Pengiriman";
                    end = "Dalam Pengiriman";
                }

                if (roleName === "Admin C5" && statusName === "Standby Pengiriman" && item.stockprodukcode === "true"&& item.deliveryunit?.toLowerCase().includes("third party")) {
                    start = "New Standby Pengiriman";
                    end = "Dalam Pengiriman";
                }
    ...
    for (const role in delayed) {
        ...
        Object.values(statusMap).forEach((st) => {
            ...
            if (cell) {
                ...
                rowhtml += `<td style="color:${color};font-weight:${fontWeight};text-align:center;vertical-align: middle;">${value}</td>`;
            } else {
                rowhtml += "<td style='vertical-align: middle;;text-align:center;'>0</td>";
            }
        });

        rowhtml += `<td style='vertical-align: middle;;text-align:center;'>${totalCount}</td><td style='vertical-align: middle;;text-align:center;'>10</td></tr>`;
        tablehtml += rowhtml;
    }
