...
socket.on('newadditemadminlist',async function(datas){
  // var dataarray=JSON.parse(document.getElementById('itemalls').dataset.json);
  ...
  //var listmessage_notif=[];//notif
  
  for (let i = 0; i < datas.length; i++) {
    const element = datas[i];
    dataarray.dataadminlist.unshift(element);
    //listidtransnew.push(element.no_transaksi);
    //var message_create=`Transakasi baru ID : ${element.id_transaksi}, ${element.item} ${element.colorvariant==''?'':`, ${element.colorvariant}`}`;//notif
    //listmessage_notif.push(message_create);//notif

    //updatedataNotif(message_create);//notif
  }
  ...
  // console.log(dataarray.dataadminlist);
  await loadingbawahupdate(dataarray.dataadminlist);

  // setTimeout(function () {
  // //notif
  //   getceknotif();
  //   for (const element of listmessage_notif) {
  //     calladdnotif(datas,element);
  //   }
  // }, 1000);
  
  //calladdnotif(datas,`Transaksi baru ID : ${listidtransnew.join(',')}`,`Transaksi baru ID : ${listidtransnew.join(',')} ditambahkan`);
...
socket.on('newprintpodo',async function(datas){
  // var dataarray=JSON.parse(document.getElementById('itemalls').dataset.json);
  let dataarray = window.dataall;
  //var listmessage_notif=[];//notif
  for (let i = 0; i < dataarray.dataadminlist.length; i++) {
    ...
    if (datas.id_transaksi==element.id_transaksi) {
      ...
      // var message_create=`Sudah print PO/DO ID : ${element.id_transaksi}, ${element.item} ${element.colorvariant==''?'':`, ${element.colorvariant}`}`;//notif
      // listmessage_notif.push(message_create);//notif
...
socket.on('newpackde-kirimbahan',async function(datas){
  // var dataarray=JSON.parse(document.getElementById('itemalls').dataset.json);
  dataarray = window.dataall;
  console.log('newpackde-kirimbahan____')
  //var listmessage_notif=[];//notif
  for (let i = 0; i < dataarray.dataadminlist.length; i++) {
    const element = dataarray.dataadminlist[i];
    if (datas.id_transaksi==element.id_transaksi) {
      ...
      window.dataall=dataarray;  // document.getElementById('itemalls').dataset.json=JSON.stringify(dataarray);
      //updatecarditem(datas,'newupholsteryselesai');
      // var message_create=`Sudah kirim bahan ke UPD untuk ID : ${element.id_transaksi}, ${element.item} ${element.colorvariant==''?'':`, ${element.colorvariant}`}`;//notif
      // listmessage_notif.push(message_create);//notif
...
socket.on('newapprovalproduk',async function(datas){
  // var dataarray=JSON.parse(document.getElementById('itemalls').dataset.json);
  console.log('newapprovalproduk')
  let dataarray = window.dataall;
  //var listmessage_notif=[];//notif
  for (let i = 0; i < dataarray.dataadminlist.length; i++) {
    const element = dataarray.dataadminlist[i];
    if (datas.id_transaksi==element.id_transaksi) {
      ...
      // var message_create=`Sudah Selesai Produksi ID : ${element.id_transaksi}, ${element.item} ${element.colorvariant==''?'':`, ${element.colorvariant}`}`;//notif
      // listmessage_notif.push(message_create);//notif
...
socket.on('newpackde-qc',async function(datas){
  ...
  for (let i = 0; i < dataarray.dataadminlist.length; i++) {
    ...
    if (datas.id_transaksi==element.id_transaksi) {
      ...
      window.dataall=dataarray; // document.getElementById('itemalls').dataset.json=JSON.stringify(dataarray);
      //updatecarditem(datas,'newupholsteryselesai');
      // var message_create=`Sudah proses qc dan packing ID : ${element.id_transaksi}, ${element.item} ${element.colorvariant==''?'':`, ${element.colorvariant}`}`;//notif
      // listmessage_notif.push(message_create);//notif
...
async function returnarray(arrayszx,prosedur) {
  ...
  for (let i = 0; i < hasil2.length; i++) {
    ...
    /* if (element.packde_qc!='true'&&element.code2.toLowerCase()=='up'&&element.stockprodukcode.toLowerCase()!='true'&&element.forcedsent!='true'&&element.approval_produkjadi!='true'||element.status.toLowerCase()=='selesai') {
  ...
  if (prosedur == 'awal') {
    ...
    arraysz.sort((a, b) => Number(b.tglinputmili) - Number(a.tglinputmili));
...
function returnarraystatusdetail(arrays) {
    ...
    return fixdatastatusitemsx.map(function (arraysx,index) {
        ...
        //console.log('arrays.stockuses=====++',JSON.parse(arrays.stockuses));
        //arrays.upholstery_fotovidbuatstock
        //${status.toLowerCase()=='selesai'?`<a href="${arrays.diterima_konsumen_img}" style="cursor:pointer;" target="_blank">${arrays.diterima_konsumen_img==''?'':'LINK'}</a>.`:''} 
        return `
        ...
        `;
        /* 
        ${arraysx.tindakan.toLowerCase()=='reject saat pengambilan item'?`<a href='/view?file=${window.btoa(`${arraysx.details.folderfoto?arraysx.details.folderfoto:arrays.reject_ambil_img}$$$-$$$${arraysx.details.keterangankerusakan}`)}' style='cursor:pointer;' target='_blank'>LINK</a>`:''} 
            
            ${arraysx.tindakan.toLowerCase()=='reject saat qc item'?`<a href='/view?file=${window.btoa(`${arraysx.details.folderfoto?arraysx.details.folderfoto:arrays.reject_qc_img}$$$-$$$${arraysx.details.keterangankerusakan}`)}' style='cursor:pointer;' target='_blank'>LINK</a>`:''} 
        
            
            ${arraysx.tindakan.toLowerCase()=='reject saat pick-up driver'?`<a href='/view?file=${window.btoa(`${arraysx.details.folderfoto?arraysx.details.folderfoto:arrays.reject_qc_img}$$$-$$$${arraysx.details.keterangankerusakan}`)}' style='cursor:pointer;' target='_blank'>LINK</a>`:''} 
            
            ${arraysx.tindakan.toLowerCase()=='reject saat diterima konsumen'?`<a href='/view?file=${window.btoa(`${arraysx.details.folderfoto?arraysx.details.folderfoto:arrays.reject_kirim_img}$$$-$$$${arraysx.details.keterangankerusakan}`)}' style='cursor:pointer;' target='_blank'>LINK</a>`:''}
...
function hitungstatus() {
  ...
  const jumlah = Array.isArray(dataarray?.dataadminlist)
    ? dataarray.dataadminlist.length
    : 0;
