...
function cekdata(arrays) {
  ...
  for (let i = 0; i < arrays.length; i++) {
    ...
    if (element.upholstery_jadi == 'true' && element.packde_ambil != 'true' && element.code2.toLowerCase() == 'up' && !element.deliveryunit.toLowerCase().includes('third party')) {
      if (`${element.bulan}${element.tahun}` == `${bulan}${tahun}`) {
        totalbelumdiambil = totalbelumdiambil + 1;
      }

    }
    if (element.upholstery_jadi == 'true' && element.packde_ambil == 'true' && element.packde_qc != 'true' && element.code2.toLowerCase() == 'up' && !element.deliveryunit.toLowerCase().includes('third party')) {
      if (`${element.bulan}${element.tahun}` == `${bulan}${tahun}`) {
        totalbelumqc = totalbelumqc + 1;
      }
  ...
  document.getElementById('perludiambiltotalshow').innerHTML = totalbelumdiambil;
...
function updatedatatotalpesanan(array, awalnotif = false) {
  ...
  if (array.upholstery_jadi != 'true' && element.packde_ambil != 'true' && array.code2.toLowerCase() == 'up' && !array.deliveryunit.toLowerCase().includes('third party')) {
...
function hitungStatusPengirimanClassy(dataadminlist, status_hoursdata) {
  ...
  return { ongoing: ongoingLate['Packde'], delayed: delayed['Packde'], warning: warning['Packde'] };
