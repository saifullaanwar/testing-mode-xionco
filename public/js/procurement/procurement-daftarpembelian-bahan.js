...
function returnarray(array,prosedur) {
    ...
    const hasilFiltered = array.filter(item => {
        // Cari data yang punya id_transaksi sama di database_belibahan
        const match = database_belibahan.find(beli => beli.id_transaksi === item.id_transaksi);

        // Jika tidak ditemukan, atau purchased-nya bukan "true", maka lolos filter
        return !match || match.purchased !== "true";
    });
    ...
    //filter data mulai juni 2025 dan seterusnya ke atas serta hanya produk code2 adalah UP yang pembuatan
    const dataFiltered = hasilFiltered.filter(item => {
        const tgl = parseInt(item.tglinputmili); // ubah ke number
        const batasAwal = new Date(2025, 5, 1).getTime(); // Juni = bulan ke-5 (0-based index)
        return (tgl >= batasAwal&&item.code2.toLowerCase() === 'up'&&item.stockprodukcode!='true'&&item.stockuses==='[]'&&item.status.toLowerCase()!='dibatalkan');
    });

    var cekIdTransaksi=[];

    for (let i = 0; i < dataFiltered.length; i++) {
        const element = dataFiltered[i];
        if (element.id_transaksi) {
    ...
    return fixarraawal.map(function(element,index){
        return `
            <tr id="dataitem-${element.id_transaksi}" data-json='${JSON.stringify(element)}'>
                ...
                <td style="text-align: center;vertical-align: middle;" >
                    <span class="badge text-bg-secondary" style="cursor: pointer;" onclick="kliktindak('${element.id_transaksi}')">Tindak</span>
                </td>
...
/**
 * @Description Search List Bahan
 */
function handleSearchListBahan(e) {
    const keyword = e.value.trim().toLowerCase(); // ambil input search
    var dataarray = window.dataall;
    var data = dataarray.dataadminlist;

    // filter data
    const hasilCari = data.filter(item => {
        return (
            (item.id_transaksi && item.id_transaksi.toLowerCase().includes(keyword)) ||
            (item.item && item.item.toLowerCase().includes(keyword)) ||
            (item.colorvariant && item.colorvariant.toLowerCase().includes(keyword))
        );
    });

    // render tabel dengan hasil pencarian
    var myobj = document.getElementById("divhpsdata");
    ...
    var divhapus = document.createElement("tbody");
    divhapus.setAttribute('id', 'divhpsdata');
    divhapus.innerHTML = returnarray(hasilCari, 'pencarian');
    datatab.appendChild(divhapus);
}
...
function kliktindak(id_transaksi) {
        ...
        history:[
            {
                ...
                "user":namalengkap,
                "tindakan":"Tindak beli dan bayar bahan UP",
                "username":username,
                "details":{
                    "tglinputmili":1750842619505,
                    "status":"Pembelian Bahan",
                    "ketstatus":`Pembelian Bahan dilakukan oleh ${namalengkap} ID: ${username}.`
                }
            }
        ]
    }
    ...
    Swal.fire({
        icon:'question',
        title:'',
        text: `Apakah bahan produk ID ${id_transaksi.split('-')[0]} sudah dibeli dan dibayar?`,
        showDenyButton: true,
...
function fizsave(datakirim) {
    fetch('/procurement/tindakpembelianbahan', {
        method: 'POST',
        headers: {
        ...
        setTimeout(function () { 
            ...
            setTimeout(function () { 
                ...
                if (resdat.icons=='success') {
                    warningpopup('success','sukses tindak');
                    
                    
                    //socket.emit('approvereqbeli',additemreqproc);
                    location.reload();
                }else if (resdat.icons=='sudah tindak sebelumnya') {
                    warningpopup('success','Data sudah pernah ditindak');
                }
                else{
                    warningpopup('error','gagal tindak');
                }
