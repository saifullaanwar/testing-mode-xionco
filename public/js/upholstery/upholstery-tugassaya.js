...
async function returnarray(arraysz1, prosedur) {
  ...
  for (let i = 0; i < hasil2.length; i++) {
    ...
    if (element.code2.toLowerCase() == 'up' && element.upholstery_jadi != 'true' && element.stockprodukcode != 'true' && element.status.toLowerCase() == 'proses produksi' && !element.notes.toLowerCase().includes('#hold') || element.status.toLowerCase() == 'proses produksi (re)' && !element.notes.toLowerCase().includes('#hold')) {
      arraysz.push(element);
    }
  } //&&!element.deliveryunit.toLowerCase().includes('third party')&&element.forcedsent!='true'
  console.log('arraysz');
  console.log(arraysz);
  /////////////////////////////
  var fixarraawal = [];
  ...
  if (prosedur=='awal') {
    var maxleng=20;  
    if (arraysz.length<20) {
      maxleng=arraysz.length;
    }
    for (let i = 0; i < maxleng; i++) {
    ...
    const nextData = arraysz.slice(currentIndex, currentIndex + itemsPerLoad);
    ...
    console.log(`curentindex : ${currentIndex} itemsPerLoad : ${itemsPerLoad} data :`);
    
    console.log(fixarraawal);

    ceklengloadmore=arraysz.length;
  } */
  if (prosedur == 'awal') {
...
function klikfilter(e) {
  ...
  let filteredProducts = dataarrays.filter(trx => {
    ...
    return searchMatch && statusMatch && itemMatch && platformMatch && upornot && upholstery_jadi;
  });
    ...
    setTimeout(function () {
      ...
      setTimeout(function () {
        ...
        if (resdat.icons == 'error') {
          ...
          window.open('/upholstery/tugassaya', '_self');
        }
      }, 800);
    }, 2000);
