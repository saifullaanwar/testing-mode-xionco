...
function fetchdata() {
  fetch('/api/adminlist', {
    ...
    .then(data => {
      ...
      //worker_threads(data);
      showtabel(data.databasevendor_proc);
      createlist()
    })
    .catch(err => {
...
function returnarray(dataitem) {
  return dataitem.sort((a, b) => {
    const vendorA = (a?.vendor || '').trim().toLowerCase()
    const vendorB = (b?.vendor || '').trim().toLowerCase()
    return vendorA.localeCompare(vendorB) 
  }).map(function (element, index) {
    const no = element?.no || "-";
    const vendor = element?.vendor?.trim() || "-";
    const tipe_bayar = element?.tipe_bayar?.trim() || "-";
    const atas_nama = element?.atas_nama?.trim() || "-";
    const rekening = element?.rekening?.trim() || "-";
    const finalElement = {
      ...element,
      vendor,
      tipe_bayar,
    ...
    return `
              ...
              <td style="text-align: left; vertical-align: middle;">${tipe_bayar}</td>
              <td style="text-align: left; vertical-align: middle;">${atas_nama}</td>
              <td style="text-align: left; vertical-align: middle;">${rekening}</td>
...
function handleSearchVendor(e){
  ...
  const dataarray = window.dataall.databasevendor_proc || []
  ...
  const filtered = dataarray.filter(item => {
    ...
    const tipeBayar = (item.tipe_bayar || '').toLowerCase()
    const atasNama = (item.atas_nama || '').toLowerCase()
    const rekening = (item.rekening || '').toLowerCase()

    return (
      vendor.includes(keyword) ||
      tipeBayar.includes(keyword) || atasNama.includes(keyword) || rekening.includes(keyword)
    )
  })
...
function renderItems() {
  ...
  newItem.innerHTML = `
    <hr>
    <b><p id="judulitem-${itemIndex}">Add New Vendor ${itemIndex}</p></b>
 <div class="col">
              <div class="input-group input-group-sm mb-3">
                <span class="input-group-text"
                  style="background-color: rgb(226, 221, 221); font-size: 12px; width: 120px;">
                  <span class="text-danger">*&nbsp;</span> Nama Vendor
                </span>
                <input type="text" list="listvendoradd-1" class="form-control" id="vendor-${itemIndex}" name="vendor" placeholder="Vendor"
                  style="font-size: 12px" oninput="oninputdata()" />
                   <datalist id="listvendoradd-1">
                    </datalist>
              </div>
              <div class="input-group input-group-sm mb-3">
                <span class="input-group-text" style="background-color: rgb(226, 221, 221); font-size: 12px; ">
                  <span class="text-danger"></span> Tipe Bayar
                </span>
                 <input type="text" list="listtipeadd-1" class="form-control" id="tipebayar-${itemIndex}" name="tipebayar" placeholder="Tipe Bayar"
                  style="font-size: 12px" oninput="oninputdata()" />
                   <datalist id="listtipeadd-1">
                    </datalist>
              </div>
              ...
              <div class="input-group input-group-sm mb-3">
                <span class="input-group-text" style="background-color: rgb(226, 221, 221); font-size: 12px; ">
                  <span class="text-danger"></span> Atas Nama
                </span>
                 <input type="text" class="form-control" id="atasnama-${itemIndex}" name="atasnama" placeholder="Atas Nama"
                  style="font-size: 12px" oninput="oninputdata()" />
              </div>
              
              <div class="input-group input-group-sm mb-3">
                <span class="input-group-text" style="background-color: rgb(226, 221, 221); font-size: 12px; ">
                  <span class="text-danger"></span> Rekening
                </span>
                 <input type="text" class="form-control" id="rekening-${itemIndex}" name="rekening" placeholder="Rekening"
                  style="font-size: 12px" oninput="oninputdata()" />
              </div>
            </div>
...
function createlist(index = 1) {
  ...
  ////// tipelist ////////////////
  var fixtipe = [];
  for (let i = 0; i < dataarray.databasevendor_proc.length; i++) {
    let element = dataarray.databasevendor_proc[i];
    if (fixtipe.indexOf(element.tipe_bayar) === -1) {
      fixtipe.push(element.tipe_bayar);
    }
  }
  tipelists_element = returnarraydata(fixtipe);
  document.getElementById(`listtipeadd-${index}`).innerHTML = tipelists_element;
}
