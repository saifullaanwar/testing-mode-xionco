...
function fetchdata() {
  fetch('/api/adminlist', {
    ...
    .then(data => {
      ...
      //worker_threads(data);
      showtabel(data.database_office);
...
function returnarray(dataitem) {
  return dataitem
    .map(function (element, index) {
      const office = element?.office?.trim() || "-";
      ...
      return `
              ...
              <td style="text-align: left; vertical-align: middle;" title="${office}">${office}</td>
...
function renderItems() {
  ...
  newItem.innerHTML = `
    <hr>
    <b><p id="judulitem-${itemIndex}">Add Office ${itemIndex}</p></b>
 <div class="col">
               <div class="input-group input-group-sm mb-3">
                  <span class="input-group-text"
                    style="background-color: rgb(226, 221, 221); font-size: 12px; width: 120px;">
                    <span class="text-danger">*&nbsp;</span> Nama Office
                  </span>
                  <input type="text" class="form-control" id="office-${itemIndex}" name="office" placeholder="Office"
                    style="font-size: 12px" oninput="oninputdata()" />
                </div>
              <!-- Note -->
              <div class="input-group1 input-group-sm mb-3">
                <span class="input-group-text" style="background-color: rgb(226, 221, 221); font-size: 12px; ">
                  <span class="text-danger">*&nbsp;</span> Alamat
                </span>
                <textarea class="form-control" id="address-${itemIndex}" name="address" placeholder="Address"
                  style="font-size: 12px; resize: none;" rows="7" oninput="oninputdata()"></textarea>
              </div>
            </div>
