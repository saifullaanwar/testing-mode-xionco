...
function fetchdata() {
    fetch('/api/adminlist', {
        ...
        //worker_threads(data);
        showtabel(data.colorvariantdata);
    })
    .catch(err => {
...
function renderItems() {
    ...
    newItem.innerHTML =`
        ...
        <span>Dibawah ini diisi jika variant adalah Fabric & CHEROKEE</span>
        <div class="input-group input-group-sm mb-3" title="Diisi untuk Variant Kain : -S = Standard, -E = Elite">
            <span class="input-group-text" style="background-color: rgb(226, 221, 221);font-size: 12px;width: 120px;"><span class="text-danger">*&nbsp;</span> Kode Kain</span>

            <input type="text" class="form-control" id="kodekain-1" name="kodekain" placeholder="Isi Buat Kode Kain (khusus Fabric & CHEROKEE)" style="font-size: 12px;" oninput="oninputdata()">
        </div>
...
function oninputdata() {
    ...
    document.querySelectorAll('#itemsContainer > .partitems').forEach((itemCard,index) => {
        let colorvariant = itemCard.querySelector('[name="colorvariant"]').value.trim();
        let kodekain= itemCard.querySelector('[name="kodekain"]').value.trim();
        //let warna= itemCard.querySelector('[name="warnakain"]').value;
...
function handleSearchVariant(e){
    ...
    let dataarray = window.dataall.colorvariantdata;
    ...
    const filteredData = dataarray.filter(item => {
        const colorVariant = item.colorvariant ? item.colorvariant.toLowerCase() : ''
        const kodekain = item.kodekain ? item.kodekain.toLowerCase() : ''
        return colorVariant.includes(searchTerm) || kodekain.includes(searchTerm)
    })
