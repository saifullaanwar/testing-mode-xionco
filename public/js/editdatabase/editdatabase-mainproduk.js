...
scrolltobuttonsidebar();
    ...
    fetchdata();
    //showtabel(dataarray.databaseallitem_web);
...
//fecth data
function fetchdata() {
  fetch('/api/editdatabase')
    .then(res => res.json())
    .then(data => {
      window.dataall = data;
      console.log('datalls',window.dataall)
      //worker_threads(data);
      showtabel(window.dataall.databaseallitem_web);
    })
    .catch(err => {
...
socket.on('newaddnew-katagori',function(datas){
    ...
    if (data.length>0) {
        for (let i = 0; i < data.length; i++) {
            ...
            dataarray.databaseallitem_katagori.push(element);
        
       
        }
        window.dataall=dataarray;//JSON.stringify(dataarray);
        console.log('dataarray.databaseallitem_web ====');
        console.log(dataarray.databaseallitem_katagori);
        loadingbawahupdate(dataarray.databaseallitem_web);
    }
