...
function fetchdata() {
  fetch('/api/adminlist', {
    ...
    .then(data => {
      ...
      //worker_threads(data);
      showtabel(data.logedit_database);
    })
    .catch(err => {
...
////
////showtabel
async function showtabel(dataarray, page = 1) {
  console.log("DATA LOG LIST: ", dataarray);
  if (page === 1) {
    // reset halaman & data global hanya saat pertama kali
    fullDataAdminList = dataarray;
...
let itemsPerPage = 50;  // Jumlah item per halaman (50 per klik)
let currentPage = 1;    // Halaman pertama
let loadedItems = 0;    // Menghitung jumlah item yang sudah dimuat
let fullDataAdminList = []; // Data yang akan ditampilkan
...
// Fungsi untuk mengembalikan HTML berdasarkan halaman yang dipilih
function returnarray(dataItem, page) {
  const sortedData = dataItem
    .sort((a, b) => {
      const tglA = parseInt(a?.tglinputmili || "0", 10);
      const tglB = parseInt(b?.tglinputmili || "0", 10);
      return tglB - tglA; // Urutkan dari tanggal paling baru
    });

  // Hitung startIndex dan endIndex berdasarkan halaman dan items per page
  const startIndex = (page - 1) * itemsPerPage;
  const endIndex = startIndex + itemsPerPage;

  // Ambil data yang sesuai dengan halaman
  const currentPageData = sortedData.slice(startIndex, endIndex);
  loadedItems = startIndex + currentPageData.length; // Update jumlah item yang dimuat

  const newHTML = currentPageData
    .map((element, index) => {
      const namalengkap = element?.namalengkap?.trim() || "-";
      let tglinputmili = element?.tglinputmili?.trim() || "-";
      const tipe = element?.tipe?.trim() || "-";
      const tindakan = element?.tindakan?.trim() || "-";

      if (tglinputmili !== "-" && !isNaN(tglinputmili)) {
        const date = new Date(parseInt(tglinputmili, 10));
        tglinputmili = date.toLocaleDateString("id-ID");
      }

      return `
        <tr data-json='${JSON.stringify(element)}' id="dataitem-${startIndex + index}">
          <td style="text-align: center; vertical-align: middle;">${startIndex + index + 1}</td>
          <td style="text-align: left; vertical-align: middle;">${tglinputmili}</td>
          <td style="text-align: left; vertical-align: middle;" title="${namalengkap}">${namalengkap}</td>
          <td style="text-align: left; vertical-align: middle;">${tipe}</td>
          <td style="text-align: left; vertical-align: middle;">${tindakan}</td>
        </tr>
      `;
    })
    .join("");
  ...
  document.getElementById('alldattab').innerHTML += newHTML;

  // Perbarui visibilitas tombol "Load More"
  setTimeout(() => {
    const btn = document.getElementById('load-more-btn');
    const dataarray = window.dataall.logedit_database
      || []
    if (btn) {
      btn.style.display = (loadedItems < dataarray.length) ? '' : 'none';
    }
  }, 100);
}

// Fungsi untuk menambah data saat tombol "Load More" diklik
function loadMoreData() {
  currentPage++;  // Tambah halaman
  returnarray(fullDataAdminList, currentPage);  // Muat data berikutnya
}

/**
 * @describe feature Searching
 */
function handleSearchLog(e) {
  ...
  const filtered = dataarray.filter(item => {
    const atasnama = (item.namalengkap || '').toLowerCase()
    const tipe = (item.tipe || '').toLowerCase()
    const tindakan = (item.tindakan || '').toLowerCase()
    return (
      atasnama.includes(keyword) || tipe.includes(keyword) || tindakan.includes(keyword)
    )
  })
