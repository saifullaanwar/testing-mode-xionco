...
function createlisthigitem() {
    ...
    var htmlx = fixarray.map((element) => {
        ...
        return `
            <option value="${element.product.trim()}">
        `
    }).join('');
...
function oninputdata() {
    document.querySelectorAll('#pagedasar').forEach((formCard, index) => {
        ...
        var data;
        var totalterjual = Math.floor(Math.random() * 50) + 1;// Math.floor(Math.random() * (900 - 100 + 1)) + 100;
            ...
            if (variantfix.length > 0) {
                ...
                /* for (let i = 0; i < variantfix.length; i++) {
                    ...
                    var hexcode=document.querySelectorAll('[name="hexcode"]')[i];
                        //variantfix.variant= "Fabric-CUSTARD",
                        //element.subvariant= "";
                        ...
                        element.imageurl= listimagename[i+5]?listimagename[i+5]:'';
...
function fizsave(datakirim) {
    fetch('/editdatabase/saveaddprodukweb', {
        method: 'POST',
        headers: {
            'Content-Type': 'application/json;charset=UTF-8',
            'Authorization': xi
        },
        body: JSON.stringify({ tipe: 'add', namalengkap, username, data: datakirim })
    })
