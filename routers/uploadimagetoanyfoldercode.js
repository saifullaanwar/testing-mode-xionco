...
router.post("/kategori", (req, res) => {
    upload(req, res, async (err) => {
        ...
        const worker = new Worker('./worker/workeruploadimagefolder.js', {
            workerData: {
                foldersaveit: 'katagoriproduk',
...
router.post("/newitemproc", (req, res) => {
    upload(req, res, async (err) => {
        if (err instanceof multer.MulterError) {
            // Tangani error spesifik dari Multer (misal: file terlalu besar, terlalu banyak file)
            if (err.code === 'LIMIT_FILE_SIZE') {
        ...
        const worker = new Worker('./worker/workeruploadimagefolder.js', {
            workerData: {
                foldersaveit: 'procurement/new_item_img',
                images: files // Mengirimkan array 'files'
            }
        });
